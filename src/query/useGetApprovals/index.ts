...
export const useGetApprovals = (): ApprovalsTx[] => {
  ...
  const Approvals: ApprovalsTx[] = data!.data.result
    ?.filter(
      (tx: ApprovalsResponseApi) =>
        tx.functionName === "approve(address _spender, uint256 _value)"
    )
    .map((tx: ApprovalsResponseApi) => {
      const decodeInput = inter.parseTransaction({
        data: tx.input,
        value: tx.value,
      });

      const infoToken: tokens1Inch | undefined =
        allTokens?.data.tokens?.[tx.to];

      const valueAllowance = ethers.utils.formatUnits(
        decodeInput.args[1],
        infoToken?.decimals
      );
      ...
      const approveTx = {
        ...
        value:
          +valueAllowance > 100000000000000000000
            ? "Unlimited"
            : valueAllowance,
