import {
  goerli,
  mainnet
} from "./chunk-BPNY4JDN.js";
import {
  __privateAdd,
  __privateGet,
  __privateMethod,
  __privateSet
} from "./chunk-Z6XFEVIA.js";
import {
  BigNumber,
  Contract,
  init_lib,
...
// node_modules/@wagmi/core/dist/chunk-SFNZNP4L.js
init_shim();
...
function normalizeFunctionName({
  ...
  const overloadFunctions = Object.keys(contract.functions).filter((x) => x.startsWith(`${functionName}(`)).map((x) => ({ name: x, fragment: import_utils.FunctionFragment.fromString(x) })).filter((x) => argsLength === x.fragment.inputs.length);
...
function isArgOfType(arg, abiParameter) {
  ...
  switch (abiParameterType) {
    case "address":
      return (0, import_utils.isAddress)(arg);
    case "bool":
...
function minimizeContractInterface(config) {
  try {
    ...
    const abi = Contract.getInterface(config.abi).format(
      import_utils2.FormatTypes.full
    );
...
async function fetchToken({
  ...
  try {
    ...
    if (err instanceof ContractResultDecodeError) {
      ...
      return {
        name: (0, import_utils4.parseBytes32String)(name),
...
async function waitForTransaction({
  ...
  try {
    ...
    if (receipt.status === 0) {
      ...
      const reason = (0, import_utils8.toUtf8String)(`0x${code.substring(138)}`);
...
// node_modules/@wagmi/connectors/dist/chunk-CYQSS77O.js
var import_utils10 = __toESM(require_utils(), 1);
...
var InjectedConnector = class extends Connector {
  constructor({
    ...
    __publicField(this, "onAccountsChanged", (accounts) => {
      if (accounts.length === 0)
        ...
        this.emit("change", {
          account: (0, import_utils10.getAddress)(accounts[0])
        });
    });
  ...
  async connect({ chainId } = {}) {
    ...
    try {
      ...
      const account = (0, import_utils10.getAddress)(accounts[0]);
  ...
  async getAccount() {
    ...
    return (0, import_utils10.getAddress)(accounts[0]);
  }
  async getChainId() {
    const provider = await this.getProvider();
    if (!provider)
  ...
  async switchChain(chainId) {
    ...
    if (!provider)
      throw new ConnectorNotFoundError();
    const id = (0, import_utils10.hexValue)(chainId);
    try {
      await Promise.all([
        provider.request({
...
//# sourceMappingURL=chunk-EJFC3QGW.js.map
